...
abstract contract MorphoTargets is BaseTargetFunctions, Properties {
    ...
    function morpho_borrow_clamped(uint256 assets, uint256 shares) public asActor {
        uint256 maxAssets = ERC20Mock(defaultMarketParams.loanToken).balanceOf(address(morpho));
        assets = between(assets, 0, maxAssets);
        morpho.borrow(defaultMarketParams, assets, shares, _getActor(), _getActor());
    }
    ...
    function morpho_flashLoan_clamped(uint256 assets, bytes memory data) public asActor {
        uint256 maxAssets = loanToken.balanceOf(address(morpho));
        assets = between(assets, 0, maxAssets);
        morpho.flashLoan(address(loanToken), assets, data);
    }
    ...
    function morpho_liquidate_clamped(uint256 seizedAssets, uint256 repaidShares, bytes memory data) public asActor {
        morpho.liquidate(defaultMarketParams, _getActor(), seizedAssets, repaidShares, data);
    }

    // Clamped handler for repay
    function morpho_repay_clamped(uint256 assets, uint256 shares, bytes memory data) public asActor {
        uint256 maxAssets = ERC20Mock(defaultMarketParams.loanToken).balanceOf(_getActor());
        assets = between(assets, 0, maxAssets);
        morpho.repay(defaultMarketParams, assets, shares, _getActor(), data);
    }
    ...
    function morpho_supply_clamped(uint256 assets, uint256 shares, bytes memory data) public asActor {
        uint256 maxAssets = ERC20Mock(defaultMarketParams.loanToken).balanceOf(_getActor());
        assets = between(assets, 0, maxAssets);
        morpho.supply(defaultMarketParams, assets, shares, _getActor(), data);
    }
    ...
    function morpho_supplyCollateral_clamped(uint256 assets, bytes memory data) public asActor {
        uint256 maxAssets = ERC20Mock(defaultMarketParams.collateralToken).balanceOf(_getActor());
        assets = between(assets, 0, maxAssets);
        morpho.supplyCollateral(defaultMarketParams, assets, _getActor(), data);
    }
